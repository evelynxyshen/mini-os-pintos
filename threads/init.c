...
main (void)
{
  ...
  // Initialize frame table inside palloc
  malloc_init ();
  paging_init ();
...
#endif
#ifdef VM
  /* frame table, supplementary page table, and swap device initation */
...
paging_init (void)
{
  ...
  /* INIT_PAGE_DIR is 0xc01010000 it is in kernel space */
  //printf("INIT_PAGE_DIR address is %x\n", (void *)init_page_dir);
  pt = NULL;
      ...
      if (pd[pde_idx] == 0)
        {
          ...
          //printf("PAGE TABLE address is %x\n", (void *)pt); //4MB RAM, only 1 pde
